...
async function main() {
  const [imagePathArg, audioPathArg, outArg] = process.argv.slice(2);

  if (!imagePathArg || !audioPathArg) {
    console.error(
      'Usage:\n' +
        '  HEYGEN_API_KEY=your_key node index.js avatar.jpg voice.mp3 [output.mp4]'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    // NEW: wait for the base avatar to fully complete
    ...
    const audioAssetId = await uploadAudioAsset(audioPath);
